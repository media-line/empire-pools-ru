jQuery(document).ready(function() {
    function formGrSend (json) {
        ...
        for (var i=0; i < json.fields.length; i++) {
            ...
            var field = jQuery('#form_back_'+json.id).find('[name=field'+i+']', this.form).val();
            console.log(field);
            if (json.fields[i]['required']) {
                if (field == '') {
                    jQuery('#form_back_'+json.id).find('[name=field'+i+']', this.form).addClass('border_red');
					error = 1;
                } else {
                    jQuery('#form_back_'+json.id).find('[name=field'+i+']', this.form).removeClass('border_red');	
                }
            }
        }
    ...
    jQuery('.js-form-send').click(function(e) {
        ...
        var json = jQuery.parseJSON(jQuery(this).closest('.mod-form-gr').find('.js-form-gr-json').val());
